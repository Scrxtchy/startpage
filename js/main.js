...
function popup(obj, msg) {
	...
	if (!visibility) {
		...
		obj.style.bottom = (obj.clientHeight * -1) + "px";
	}
...
function main() {
	...
	HelpText = "<table><tr><td>" +
		...
		"-config</td><td>: Opens the config menu</td></tr></table>";

	// generate helptext for custom options
	var searchsquareOptions = data.squares[data.squares.length - 1].inputs;
	if (searchsquareOptions) {
		HelpText += "<br><table>";
		
			Object.keys(searchsquareOptions).forEach(function(prefixItem){
			Object.keys(searchsquareOptions[prefixItem]).forEach(function(searchItem) {
				HelpText += "<tr><td>" + prefixItem + searchItem + "</td><td>: " + searchsquareOptions[prefixItem][searchItem].name + "</td></tr>";
			})
		})
		HelpText  += "</table>";
	}
